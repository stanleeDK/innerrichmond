...
namespace :ingest do 
	...
	def coordinate_scraping_of_entire_site
		...
		for j in 0..100 #ages 
			for i in 1..45 # neighbourhoods 
				puts i
				params[:providers][:neighborhood_ids][0] = i 
				params[:providers][:ages][0] = j 

				# puts params 
				commence_scraping(params) 
			end 
		end 

	end 

	def commence_scraping (params)
	...
	task :scrape_ccsf,[:nghb] => :environment do |task,args|
		coordinate_scraping_of_entire_site()
	end 
end 
